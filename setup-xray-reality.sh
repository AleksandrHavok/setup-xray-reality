...
# ── [3] ПРОВЕРКА КОНФИГА И БЭКАП ──
[ ! -f "$CONF" ] && { log "[ERROR] Конфиг не найден: $CONF"; exit 1; }
BACKUP="${CONF}.bak.$(date '+%Y%m%d%H%M%S')"
cp "$CONF" "$BACKUP"

# ── [4] ГЕНЕРАЦИЯ НОВЫХ ДАННЫХ (SID, UUID) ──
...
# ── [5] ОСНОВНАЯ ЛОГИКА: FIRST RUN или ADD DEVICE ──
if [ -z "$EXISTING_UUID" ] || [ "$EXISTING_UUID" = "null" ]; then
    # === ПЕРВЫЙ ЗАПУСК: генерируем ключи и первого клиента ===
    ...
    # === ПОВТОРНЫЙ ЗАПУСК: только добавляем новый SID ===
