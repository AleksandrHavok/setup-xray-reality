...
# ── 0. Синхронизация времени (Reality критичен) ──
...
# ── 6. Validate & restart ──
if ! xray test -config "$CONF" > /dev/null 2>&1; then
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    curl -fsSL "${REPO_RAW}/xray-auto-update.sh" -o /usr/local/bin/xray-auto-update.sh 2>/dev/null && {
        ...
        if ! crontab -l 2>/dev/null | grep -q "xray-auto-update.sh"; then
            (crontab -l 2>/dev/null || echo ""; echo "0 4 * * 1 /usr/local/bin/xray-auto-update.sh") | crontab -
